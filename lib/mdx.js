...
export async function readMarkdownFile(absPath) {
  ...
  // Expose frontmatter both nested and flattened so pages can safely use top-level fields
  return {
    frontmatter,
    html,
    slug,
...
export async function getIssueMeta() {
  ...
  return items.map(({ frontmatter, slug, title, description, date }) => ({
    slug,
    title: title ?? frontmatter.title,               // always defined now
    description: description ?? frontmatter.description ?? '',
    date: date ?? frontmatter.date ?? null,
  }));
}
